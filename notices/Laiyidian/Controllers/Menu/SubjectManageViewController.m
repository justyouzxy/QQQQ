...
@property (weak, nonatomic) FistViewController *parentController;
...
//返回对象
+ (id)initWithSelectedSubjectAndParentController:(NSArray *)subjectArr parentController:(FistViewController *)parentController {
...
//    [label addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-(0)-[btn(40)]-|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(btn)]];
//    [label addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(0)-[btn]-|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(btn)]];
//
...
-(void)createSelectedBar:(UIView *)contentView {
    ...
    //靠右
    sLabel2.textAlignment = NSTextAlignmentRight;
...
//    // TODO
//    NSInteger total = self.unSelectedSubjectArr.count;
//    float x = 10; // button的X坐标
//    float y = 10; // button的Y坐标
//    float step_x = 10; // X方向递进宽度
//    float step_y = 15; // Y方向递进高度
//    float btn_width = 90;
//    float btn_height = 30;
//    int btnAtRowNum = 4; // 每一行btn数量
//    
//    for(int i = 0; i < total; i++) {
//        // 如果需要换行，则修改y坐标
//        if(i != 0 && i % btnAtRowNum == 0) {
//            y += btn_height + step_y;
//            x = 10;
//        }
//        
//        CGRect frame = CGRectMake(x, y, btn_width, btn_height);
//        // 计算出下一个点的坐标
//        x += btn_width + step_x;
//        
//        UIButton* btn = [UIButton buttonWithType:UIButtonTypeCustom];
...
//    NSLog(@"%@", self.parentController);
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(getSelectArray:)]) {
        [_delegate getSelectArray:self.selectedSubjectArr];
    }
