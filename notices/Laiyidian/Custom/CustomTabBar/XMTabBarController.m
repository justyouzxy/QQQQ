...
- (id)initWithItemNormalImages:(NSMutableArray *)normal
                SelectedImages:(NSMutableArray *)selected
...
- (id)initWithItemNormalImages:(NSMutableArray *)normal SelectedImages:(NSMutableArray *)selected titles:(NSMutableArray *)titles{
    if ([super init]) {
        
        self.tabBarView = [[TabBarView alloc] initWithItemNormalImages:normal SelectedImages:selected titles:titles];
...
#warning XXXX
- (void)popViewShow {
    _contentView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 300, 100)];
    _contentView.backgroundColor = [UIColor clearColor];

    UIButton *leftButton = [[UIButton alloc]initWithFrame:CGRectMake(10, 0, _contentView.frame.size.width / 2 - 20, 100)];
    [leftButton setTitle:@"发布需求" forState:UIControlStateNormal];
    [leftButton setBackgroundColor:[UIColor orangeColor]];
    [leftButton addTarget:self action:@selector(releaseDemand) forControlEvents:UIControlEventTouchUpInside];
    
    UIButton *rightButton = [[UIButton alloc]initWithFrame:CGRectMake(_contentView.frame.size.width / 2 + 10, 0, _contentView.frame.size.width / 2 - 20, 100)];
    [rightButton setTitle:@"发布技能" forState:UIControlStateNormal];
    [rightButton setBackgroundColor:[UIColor orangeColor]];
    [rightButton addTarget:self action:@selector(releaseSkills) forControlEvents:UIControlEventTouchUpInside];
...
- (void)releaseDemand {
    NeedViewController *need = [NeedViewController new];
//    [self.selectedViewController addChildViewController:need];
    [self.selectedViewController pushViewController:need animated:YES];

    [[HWPopTool sharedInstance] closeWithPresentVIew:_contentView animated:YES];
...
#warning centerButtonH W
    /** 中间按钮的宽高 */
    CGFloat centerButtonWH = 48.5;
    /** 普通按钮的高度 */
...
-(instancetype)initWithFrame:(CGRect)frame{
    if ([super initWithFrame:frame]) {
        ...
        self.badgeValueLabel.backgroundColor = [UIColor orangeColor];
